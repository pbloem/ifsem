...
import static nl.peterbloem.kit.MatrixFunctions.getDeterminant;
import static nl.peterbloem.kit.MatrixFunctions.matrix;
...
import nl.peterbloem.kit.data.Datasets;
import nl.peterbloem.kit.data.MVN;
import nl.peterbloem.kit.data.Point;
import nl.peterbloem.kit.data.Rotation;
...
public class EM
{
	...
	{		
		...
		expectationInner(depths, model, data, z, zk, p, pk, t, code, 
				1.0, MatrixFunctions.identity(model.dimension()), 
	...
	{		
		data = post.inverse().map(data);
		RealMatrix y = MatrixFunctions.matrix(data);
	...
	public static RealVector zFromData(int k, int maxDepth, List<Point> data, Similitude post, RealMatrix p)
	{
		...
		{			
			RealMatrix yCentered = 
					y.subtract(
						outer(t.getColumnVector(j), 
						ones(y.getColumnDimension()))
						);
			...
			RealMatrix t, RealVector z)
	{
		...
		{						
			...
			RealVector yWeights = pkzNorm.operate(ones(pkz.getColumnDimension()));
			RealVector yMean = y.operate(yWeights);
			
			RealVector tWeights = pkzNorm.preMultiply(ones(pkz.getRowDimension()));
			...
			RealVector c = MatrixFunctions.ones(a.getColumnDimension());
			double last = MatrixFunctions.getDeterminant(svd.getU().multiply(svd.getVT()));
			c.setEntry(c.getDimension()-1, last);
			
			RealMatrix rot = svd.getU().multiply(MatrixFunctions.diag(c)).multiply(svd.getVT());
			...
			// * Find the scale
			double sa, sb, sc;
			sa = Similitude.inTrace(yCentered, pkz.operate(MatrixFunctions.ones(pkz.getColumnDimension())), yCentered);
	...
	public static Similitude maximizePost(List<Point> data, RealMatrix p, RealVector z, RealMatrix t)
	{
		...
		RealVector xWeights = pzNorm.operate(MatrixFunctions.ones(pz.getColumnDimension()));
		RealVector xMean = x.operate(xWeights);
		
		RealVector tWeights = pzNorm.preMultiply(MatrixFunctions.ones(pz.getRowDimension()));
		RealVector tMean = t.operate(tWeights);
		...
		double last = getDeterminant(svd.getU().multiply(svd.getVT()));
		c.setEntry(c.getDimension()-1, last);
		...
		// * Find the scale
		double sa, sb, sc;
		sa = Similitude.inTrace(xCentered, pz.operate(ones(pz.getColumnDimension())), xCentered);
