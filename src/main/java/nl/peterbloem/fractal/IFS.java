...
import nl.peterbloem.kit.data.AffineMap;
import nl.peterbloem.kit.data.MVN;
import nl.peterbloem.kit.data.Map;
import nl.peterbloem.kit.data.MapModel;
import nl.peterbloem.kit.data.Point;
import nl.peterbloem.kit.data.Similitude;
import nl.peterbloem.kit.data.Weighted;
import nl.peterbloem.kit.data.WeightedLists;
	...
	implements Serializable, Parametrizable
{
	 ...
	 * 
	 * Note that samples from this generator are _not_ independent. Subsequent 
	 * points can always be mapped to one another by one of the components of the
	 * IFS (which is not true for independent samples). If independence is important,
	 * shuffle the points after sampling or sample from generator(double) with 
	 * a high depth.
	 * 
	 * @return
	 */
	...
	private class IFSFixedDepthGenerator extends AbstractGenerator<Point>
	{
		...
		public Point generate()
		{
			...
			int intDepth = (int)Functions.probRound(depth);
	...
	private class IFSMixedDepthGenerator extends AbstractGenerator<Point>
	{
		...
		public Point generate()
		{
			...
			int depth = Functions.choose(depths, 1.0);
	...
	{	
		SearchResult res = search(
				...
				MatrixFunctions.identity(ifs.dimension()), new ArrayRealVector(ifs.dimension()),
				MatrixFunctions.identity(ifs.dimension()), new ArrayRealVector(ifs.dimension()),
				basis);
		return res;
	}
	...
	private static class SearchResultImpl implements SearchResult
	{
		...
		public void show(double logProb, List<Integer> code, Point point, Point mean, double logPrior)
		{
			...
			approx = Functions.logSum(Math.E, logPrior, approx);
			densityApprox = Functions.logSum(Math.E, densityApprox, approx);
